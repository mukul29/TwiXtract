...
class PresentUserData:
	...
	def make_tweets_list(self):
		for line in open(self.fetched_tweets_filename):
			try:
				self.tweets_list.append(json.loads(line))
			except:
				continue
	...
	def show_dataframe(self, html_file_path):
		#print(self.df)
	...
	def retweets_sentiment_scatter(self):
		...
		plt.scatter(self.df.sentiment_value, self.df.retweet_count, c=np.random.rand(number_of_rows), s=30)
...
if __name__ == '__main__':
	...
	presentUserData = PresentUserData(fetched_tweets_filename)
	presentUserData.make_tweets_list()
	presentUserData.make_dataframe()
	presentUserData.show_dataframe()
	presentUserData.retweets_sentiment_scatter()
	presentUserData.sentiment_pie()
	presentUserData.source_bar()
	presentUserData.language_bar()
